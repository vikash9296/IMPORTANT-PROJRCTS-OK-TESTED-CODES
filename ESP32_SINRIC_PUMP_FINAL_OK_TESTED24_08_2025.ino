...
bool onPowerState(const String &deviceId, bool &state) {
  if (deviceId == DEVICE_ID_1) {
    ...
    pulseRelay(RELAY2_PIN);   // Always just 2s trigger
...
void startAPMode() {
  WiFi.softAP("PumpController_AP","12345678");//SSID + Password
...
void loop() {
  ...
  // ---------------- Manual button trigger (5s hold = 2s pulse) ----------------
  if (digitalRead(MANUAL_BTN_PIN) == LOW) {
    if (!btnPressed) {
      ...
      // Update Sinric state event (just for sync)
      if (WiFi.status() == WL_CONNECTED) {
        SinricProSwitch &sw2 = SinricPro[DEVICE_ID_2];
